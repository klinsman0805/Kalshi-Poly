...
async def _handle(cmd):
    ...
    if action == "scoring":
        # Is this order ACTUALLY earning rewards right now? Resting inside the
        # band is not sufficient — an order below the market's (intraday-
        # mutable) rewards_min_size scores zero while keeping full fill risk.
        # This is the CLOB's own authoritative answer.
        s = await client.get_order_scoring(order_id=cmd["order_id"])
        scoring = getattr(s, "scoring", s)
        return {"ok": True, "scoring": bool(scoring)}
