...
from feeds.gfs_ensemble import GfsEnsembleFeed
...
# Lows have never filled (0/0 vs highs filling fine): the daily min locks in at
# dawn and is usually already priced to ~100c by the time our gates would fire,
# and on the rare thin-but-priceable book, a full STAKE_USD-sized FOK asks for
# more than the book has and gets killed outright (Seoul low: 173 misses in a
# day). Behind a flag pending paper validation — see the daily-low investigation
# in memory for the full analysis before enabling live.
WEATHER_LOW_LIQ_SIZE = os.getenv("WEATHER_LOW_LIQ_SIZE", "false").strip().lower() == "true"
# Master switch for low-temperature trading, independent of WEATHER_LOW_LIQ_SIZE
# above. Off = highs only. Lows are still fully computed and shown on the
# dashboard (model p, edge, signal reasoning) for visibility — this only
# intercepts the final ENTER so a paused low is clearly distinguishable from
# one the market/model actually rejected.
WEATHER_LOWS_ENABLED = os.getenv("WEATHER_LOWS_ENABLED", "true").strip().lower() == "true"
...
_GROUPS = [
    (0, "actionable",   ["ENTER"]),
    # would have entered — WEATHER_LOWS_ENABLED=false is holding it back, not
    # the market or the model. Ranked right after actionable: this is the
    # signal you'd most want to notice at a glance.
    (1, "lows-paused",  ["LOWS-OFF"]),
    # model likes it; the MARKET is the blocker — these can flip within minutes
    (2, "market-blocked", ["THIN-EDGE", "PRICED", "TOO-GOOD", "WIDE",
                           "NO-DEPTH", "NO-BOOK", "MKT-LOCKED"]),
    # the day itself isn't decided yet — waiting on the weather, not the book
    (3, "not-yet",      ["NOT-LOCKED", "NO-LOCK", "EARLY"]),
    (4, "no-data",      ["NO-DATA"]),
    (5, "other-day",    ["WAIT"]),
    (6, "untradeable",  ["MONITOR"]),
]
...
class WeatherEngine:
    def __init__(self, metar, executor=None, on_log=None):
        ...
        # observation-anchored ensemble — SHADOW ONLY, gates nothing. Refreshes
        # itself every few hours; costs one batched HTTP call. See
        # feeds/gfs_ensemble for why the LEVEL bias is the part worth measuring.
        self.gfs = GfsEnsembleFeed(self.on_log)
        self.climo = {}
    ...
    def refresh(self):
        ...
        self.metar.poll()
        # after METAR, so the anchor always uses the observations this very
        # cycle will trade on. Swallows its own errors — shadow data must never
        # be able to stop a refresh.
        self.gfs.poll(self.metar.snapshot())
    ...
    def _ens_brief(self, icao):
        """Headline ensemble numbers for the dashboard row — how many members
        are biased which way, and by how much. The full block goes on the entry
        record, where it's written once instead of polled."""
        e = self.gfs.station(icao) if icao else None
        if not e:
            return None
        return {"n_members": e["n_members"],
                "members_cold": e["members_cold"], "members_warm": e["members_warm"],
                "bias_med_c": e["bias_med_c"], "bias_med_f_delta": e["bias_med_f_delta"]}
    ...
    def _compute_event(self, e):
        ...
        if signal == "ENTER" and best is not None and BOOK_CONFIRM:
            signal, why = self._book_confirm(best, kind)
        ...
        return {
            ...
            # weather-regime signals (shadow measurement — see metar_conditions)
            "conditions": (st or {}).get("conditions"),
            # anchored ensemble bias (shadow measurement — see gfs_ensemble).
            # Brief form only: the full block is ~30 fields and the dashboard
            # re-fetches every row on a short poll.
            "ensemble": self._ens_brief(icao),
            "buckets": buckets,
            ...
            "entry": ({\
                ...
                "neg_risk": e["neg_risk"],
                # decline-at-entry measurement (see weather_exec._consider):
                # the extreme and the CURRENT reading at signal time, market unit
                "ext_c": ext, "temp_now": temp_now,
                # weather-regime signals at signal time (shadow measurement)
                "conditions": (st or {}).get("conditions"),
                # anchored ensemble bias at signal time (shadow measurement)
                "ensemble": self.gfs.station(icao) if icao else None,
    ...
    def _book_confirm(self, best, kind=None):
        ...
        if (vwap is None or got + 1e-9 < shares) and WEATHER_LOW_LIQ_SIZE and kind == "low" \
                and got >= (best.get("min_size") or 5):
            # Full STAKE_USD sizing is what's been losing every low signal to
            # NO-DEPTH — the book has SOMETHING, just less than we asked for.
            # Size down to what's actually there instead of walking away empty.
            shares = math.floor(got)
            vwap, got, marginal = vwap_for_size(asks, shares)
    ...
    def _gate(self, e, st, is_today, tradeable, best, ext):
        ...
        if e["kind"] == "low" and not WEATHER_LOWS_ENABLED:
            return "LOWS-OFF", (f"would ENTER (p {best['p']:.2f} @ {best['ask_c']:.0f}c) "
                                 "— lows paused, highs only")
    ...
    def state(self):
        return {
            ...
            "config": {"p_min": P_MIN, "price_max_c": PRICE_MAX_C,
                       ...
                       "low_liq_size": WEATHER_LOW_LIQ_SIZE,
                       "lows_enabled": WEATHER_LOWS_ENABLED},
        }
