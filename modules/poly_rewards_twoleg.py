...
# Unconditional hedging treats every fill as a mistake. It isn't: a fill on the
# side the METAR data FAVORS is a winner bought at a discount, and hedging it
# pays a taker fee to convert ~+12c/share into ~$0. Holding instead keeps that
# edge, protected by the same monotonic dead-bucket stop the weather bot uses.
# Default OFF: while off, every completion still records the counterfactual
# (which side was favored, what holding would have been worth) so the ledger
# measures whether this is really better BEFORE it is switched on.
CONDITIONAL_COMPLETE = os.getenv("POLY_TWOLEG_CONDITIONAL_COMPLETE", "false").strip().lower() == "true"
# Ceiling on total capital tied up in held (un-hedged) favored legs at once.
MAX_HELD_USD = float(os.getenv("POLY_TWOLEG_MAX_HELD_USD", "20"))
...
class PolyTwoLeg:
    ...
    def __init__(self, on_log=None, exec_=None):
        ...
        self._breaker_logged = False   # log the circuit breaker once, not every cycle
    ...
    def cycle(self, scan_results=None):
        # Held favored legs outlive the position that created them, so they are
        # watched every cycle regardless of what the LP slot is doing.
        try:
            self.check_holdings()
        except Exception as e:  # noqa: BLE001
            self.on_log("!", f"[twoleg] holdings check failed: {e}")
    ...
    def _maybe_enter(self, scan_results):
        if not scan_results:
            return
        # Circuit breaker: a day that has already lost the limit must not open
        # anything new. Checked at ENTRY only — positions already open are
        # still managed to completion, because abandoning them mid-flight would
        # strand exposure, which is the very thing this protects against.
        ok, reason = check_breaker()
        if not ok:
            if not self._breaker_logged:
                self.on_log("✗", f"[twoleg] CIRCUIT BREAKER: {reason}")
        ...
        # require_confidence=False: see PolyRewardsExec.check. The estimate's
        # accuracy is irrelevant to this path's safety (lock + consensus + wide
        # two-sided quotes + auto-complete + capital cap), and demanding "ok"
        # left the tradeable set empty because thin books are precisely what a
        # decided bucket has.
        cands = self.exec.check(scan_results, require_confidence=False)
    ...
    def _bucket_state(self, condition_id, kind, station):
        """Live weather verdict for one bucket: which side the DATA favors and
        whether the bucket is already dead. Returns None when it cannot be
        determined (feed miss, no station reading, missing metadata) — callers
        must then fall back to unconditional hedging, never to holding.

        `favored_side` is "yes" when the running extreme sits inside the
        bucket and "no" when it is outside; `dead` means the extreme has moved
        past the bucket on the monotonic side, which makes YES worthless and
        NO certain. All comparisons run in the BUCKET'S unit (see
        poly_rewards_exec.observed_extreme — Celsius was hardcoded here until
        2026-08-04 and silently broke every °F market).
        """
        ...
        try:
            ...
            self.on_log("!", f"[twoleg] weather feed unavailable for hold check: {e}")
            return None
        bucket = None
        for e in events:
            for b in e.get("buckets", []):
                if b.get("condition_id") == condition_id:
                    bucket = b
        if not bucket:
            return None
        lo, hi, unit = bucket.get("lo"), bucket.get("hi"), bucket.get("unit")
        try:
            self.exec.metar.set_stations({station: self.exec._climo_tz.get(station, "")})
            self.exec.metar.poll()
            st = self.exec.metar.snapshot().get(station)
        except Exception as e:  # noqa: BLE001
            self.on_log("!", f"[twoleg] METAR unavailable for hold check: {e}")
            return None
        ext = observed_extreme(st, kind, unit)
        if ext is None:
            return None
        favored = bucket_favored(ext, kind, lo, hi)
        return {
            "lo": lo, "hi": hi, "unit": unit, "ext": ext,
            "dead": bucket_dead(ext, kind, lo, hi),
            "favored_side": "yes" if favored else "no",
            "trend_adverse": _trend_continuing_adverse((st or {}).get("today_obs") or [],
                                                        kind, lo, hi),
        }

    def _held_capital_usd(self):
        """USD currently tied up in un-hedged held legs (ledger replay)."""
        if not TWOLEG_LOG.exists():
            return 0.0
        open_h = {}
        for line in open(TWOLEG_LOG):
            ...
            if r.get("type") == "twoleg_holding":
                ...
                open_h.pop(r.get("hold_id"), None)
        return sum(h["size"] * h["price_c"] / 100.0 for h in open_h.values())
    ...
    def _requote(self, reason, cancel_first):
        ...
        if not plan:
            ...
            return
        # A requote is a NEW entry decision, not a price tweak — re-apply the
        # entry gates against the market as it is NOW. Both of these were
        # missing and together produced a real -$17.45 paper loss on 2026-08-04:
        #  - the capital cap: min_size rose 20 -> 100 mid-position and the
        #    requote silently placed $95 of capital against a $20 ceiling, so
        #    the eventual adverse fill was 5x the size it should ever have been;
        #  - the consensus gate: post-print requotes re-quoted blindly at
        #    whatever the new mid was. That market's mid walked 4c -> 27c across
        #    prints, i.e. it had become CONTESTED, and we kept quoting it. Drift
        #    alone could not catch this because _requote resets the reference
        #    mid each time, so a trend that arrives in steps never trips
        #    ABANDON_C.
        if plan["capital_usd"] > MAX_CAPITAL_USD:
            self.on_log("✗", f"[twoleg] requote would need ${plan['capital_usd']:.0f} "
                             f"(min size {plan['size']:.0f} sh) > ${MAX_CAPITAL_USD:.0f} cap "
                             f"— closing instead of scaling up")
            self._close("requote_over_budget")
            return
        if max(plan["yes_mid_c"], plan["no_mid_c"]) < CONSENSUS_MIN_C:
            self._blacklist.add(pos["condition_id"])
            self.on_log("✗", f"[twoleg] market became CONTESTED while we quoted it "
                             f"(mids {plan['yes_mid_c']:.0f}/{plan['no_mid_c']:.0f}c, need one "
                             f">= {CONSENSUS_MIN_C:.0f}c) — abandoning, not re-quoting into it")
            self._close("thesis_broke_became_contested")
            return
        yes_oid = place_limit(plan["yes_token"], plan["yes_bid_c"], plan["size"],
                              side="BUY", post_only=True, live=POLY_TWOLEG_LIVE)
        no_oid = place_limit(plan["no_token"], plan["no_bid_c"], plan["size"],
    ...
    def _auto_complete(self, filled_side, other_asks):
        ...
        # ── conditional hold ────────────────────────────────────────────────
        # A fill on the side the DATA favors is a winner bought at a discount,
        # not an accident to be undone. Hedging it pays a taker fee to convert
        # that edge into ~$0. Hold instead — but only on a live, unambiguous
        # read: any uncertainty (feed miss, dead bucket, adverse trend, budget)
        # falls through to the unconditional hedge, which is always safe.
        state = self._bucket_state(pos["condition_id"], pos.get("kind"), pos.get("station"))
        filled_c = pos[f"{filled_side}_bid_c"]
        held_size = pos.get(f"{filled_side}_matched") or pos["size"]
        hold_ok = False
        if state:
            favored = state["favored_side"] == filled_side
            # A YES leg is only holdable while the bucket is alive; a NO leg is
            # SAFEST precisely when the bucket is dead (the extreme has passed
            # it, so NO is mathematically certain).
            survivable = (not state["dead"]) if filled_side == "yes" else True
            room = self._held_capital_usd() + held_size * filled_c / 100.0 <= MAX_HELD_USD
            hold_ok = favored and survivable and not state["trend_adverse"] and room

        if CONDITIONAL_COMPLETE and hold_ok:
            self._hold_favored(filled_side, held_size, filled_c, state)
            return
        ...
        pos[f"{other}_filled"] = True
        pos["completed"] = True
        # Counterfactual: what the conditional-hold path WOULD have decided
        # here, and what holding would pay if the favored side resolves. With
        # CONDITIONAL_COMPLETE off this is how the ledger measures whether
        # holding actually beats hedging, before the flag is ever flipped.
        hold_upside = (100.0 - paid_c) / 100.0 * size
        self._persist({"type": "twoleg_completed", "id": pos["id"],
                      "ts": datetime.now(timezone.utc).isoformat(),
                      ...
                      "favored_side": (state or {}).get("favored_side"),
                      "bucket_dead": (state or {}).get("dead"),
                      "filled_was_favored": bool(state and state["favored_side"] == filled_side),
                      "would_have_held": bool(hold_ok),
                      "hold_upside_usd": round(hold_upside, 4),
                      "hold_downside_usd": round(-paid_c / 100.0 * size, 4)})
    ...
    def _hold_favored(self, filled_side, size, price_c, state):
        """Keep a favored-side fill instead of hedging it away, and FREE THE
        LP SLOT so quoting continues elsewhere.

        The held shares are tracked in their own ledger records rather than in
        the position, because the LP position's job (resting a pair) is over
        the moment a leg fills and the mid moves. _check_holdings() then
        watches them under the monotonic dead-bucket stop until they resolve
        or die.
        """
        pos = self.pos
        # We are deliberately directional now — the sibling must not fill and
        # silently neutralize the edge we just chose to keep.
        cancel_limit(pos.get(f"{'no' if filled_side == 'yes' else 'yes'}_order_id"),
                     live=POLY_TWOLEG_LIVE)
        hold_id = f"{pos['id']}-hold-{filled_side}"
        self._persist({
            "type": "twoleg_holding", "hold_id": hold_id, "id": pos["id"],
            "ts": datetime.now(timezone.utc).isoformat(),
            "condition_id": pos["condition_id"], "question": pos.get("question"),
            "token": pos[f"{filled_side}_token"], "side": filled_side,
            "size": size, "price_c": price_c,
            "city": pos.get("city"), "kind": pos.get("kind"),
            "station": pos.get("station"), "date": pos.get("date"),
            "lo": state["lo"], "hi": state["hi"], "unit": state["unit"],
            "ext_at_hold": state["ext"], "bucket_dead_at_hold": state["dead"],
            "mode": "live" if POLY_TWOLEG_LIVE else "paper",
        })
        upside = (100.0 - price_c) / 100.0 * size
        self.on_log("✓", f"[twoleg] HOLDING favored {filled_side.upper()} leg — "
                         f"{size:.0f} sh @ {price_c:.0f}c on {pos.get('city')} "
                         f"{pos.get('kind')} (obs {state['ext']}{state['unit']}, bucket "
                         f"{state['lo']}-{state['hi']}); redeems +${upside:.2f} if it "
                         f"holds, stopped out if the bucket dies")
        self._close(f"held_favored_{filled_side}")

    def _open_holdings(self):
        if not TWOLEG_LOG.exists():
            return {}
        open_h = {}
        for line in open(TWOLEG_LOG):
    ...
    def check_holdings(self):
        """Watch every held favored leg under the monotonic dead-bucket stop.

        Called once per cycle, independently of the LP position slot — holdings
        outlive the position that created them. A holding dies when the daily
        extreme moves past its bucket (or a consistent adverse trend forms);
        then the shares are sold into whatever bid exists, which is a real loss
        but a bounded one. A holding whose market has left the feed has almost
        certainly resolved — it is closed at face value and reconciled by the
        supervisor against the wallet's actual positions.
        """
        holdings = self._open_holdings()
        if not holdings:
            return
        for hold_id, h in holdings.items():
            state = self._bucket_state(h["condition_id"], h.get("kind"), h.get("station"))
            if state is None:
                # market gone from the feed => almost certainly resolved. YES
                # held to resolution pays 100c if the bucket was alive; a NO
                # held on a dead bucket pays 100c. Record at face value and let
                # the supervisor reconcile against real positions.
                won = (h["side"] == "no" and h.get("bucket_dead_at_hold")) or \
                      (h["side"] == "yes" and not h.get("bucket_dead_at_hold"))
                proceeds = h["size"] * (1.0 if won else 0.0)
                self._close_holding(hold_id, h, "resolved_off_feed",
                                    proceeds_usd=proceeds, sold_c=100.0 if won else 0.0)
                continue
            # A YES holding dies when the extreme passes the bucket; a NO
            # holding dies only if the bucket comes back to life, which the
            # monotonic rule forbids once dead — so NO holdings are terminal
            # winners and simply wait for resolution.
            if h["side"] == "yes" and (state["dead"] or state["trend_adverse"]):
                self._exit_holding(hold_id, h, "dead" if state["dead"] else "trend")

    def _exit_holding(self, hold_id, h, reason):
        """Sell a holding that has gone against us, into whatever bid exists."""
        try:
            bids, _ = _fetch_book(h["token"])
        except Exception as e:  # noqa: BLE001
            self.on_log("!", f"[twoleg] book fetch failed exiting holding: {e}")
            return
        bid_c = bids[0][0] if bids else 0.0
        if bid_c <= 0:
            self.on_log("!", f"[twoleg] holding {hold_id[:20]} is dead ({reason}) but the "
                             f"book has NO bid — cannot exit, will retry next cycle")
            return
        # Cross down to take the bid now; this is an emergency unwind, not a
        # price-improvement exercise.
        sell_c = max(1.0, bid_c - 2)
        oid = place_limit(h["token"], sell_c, h["size"], side="SELL",
                          post_only=False, live=POLY_TWOLEG_LIVE)
        if POLY_TWOLEG_LIVE and not oid:
            self.on_log("!", f"[twoleg] holding exit FAILED for {hold_id[:20]} — "
                             f"still exposed, retrying next cycle")
            return
        self._close_holding(hold_id, h, reason, proceeds_usd=h["size"] * bid_c / 100.0,
                            sold_c=bid_c, order_id=oid)

    def _close_holding(self, hold_id, h, reason, proceeds_usd, sold_c, order_id=None):
        cost = h["size"] * h["price_c"] / 100.0
        pnl = proceeds_usd - cost
        self._persist({"type": "twoleg_holding_closed", "hold_id": hold_id,
                      "ts": datetime.now(timezone.utc).isoformat(), "reason": reason,
                      "sold_c": sold_c, "cost_usd": round(cost, 4),
                      "proceeds_usd": round(proceeds_usd, 4), "pnl_usd": round(pnl, 4),
                      "order_id": order_id})
        self.on_log("✓" if pnl >= 0 else "✗",
                   f"[twoleg] holding closed ({reason}) {h.get('city')} {h.get('kind')} "
                   f"{h['side'].upper()} — cost ${cost:.2f}, proceeds ${proceeds_usd:.2f}, "
                   f"pnl ${pnl:+.2f}")
