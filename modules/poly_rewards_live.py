...
def order_scoring(order_id, live=None):
    """Is this resting order ACTUALLY earning rewards? True/False from the
    CLOB, or None when unknown (paper, missing id, or lookup failure).

    Resting inside the reward band is NOT sufficient to earn: an order below
    the market's `rewards_min_size` scores zero while still carrying full fill
    risk, and min_size changes intraday (observed 2026-08-04: a market went
    20 -> 100 shares minutes after a pair was placed, silently zeroing it).
    None means UNKNOWN — callers must not treat it as "not earning" and act
    destructively on it.
    """
    if not order_id:
        return None
    ...
    if not armed:
        return None
    res = _run_sdk({"action": "scoring", "order_id": order_id})
    if not res.get("ok"):
        log.warning("[lp-live] scoring lookup failed for %s: %s",
                    str(order_id)[:16], res.get("error") or res)
        return None
    return bool(res.get("scoring"))
