...
def observed_extreme(snap, kind, unit):
    """The station's running daily extreme IN THE BUCKET'S OWN UNIT.

    Bucket bounds come from the market question, so a US market's lo/hi are
    whole °F while an international one's are °C — and MetarFeed publishes the
    running extreme in both (max_c/min_c and max_f/min_f). Reading the Celsius
    field and comparing it against °F bounds silently corrupts every
    dead/alive decision: a Miami 92-93°F bucket would compare an observed 33
    against 92, so `33 > 93` is never true and the high-side stop-loss could
    never fire, while the low-side would read as dead instantly.

    This mirrors the selection modules/weather.py:284 already does for the
    trading bot; callers here were hardcoding Celsius until 2026-08-04.
    Returns None when the station has no reading yet.
    """
    if not snap:
        return None
    if (unit or "C").upper() == "F":
        return snap.get("max_f") if kind == "high" else snap.get("min_f")
    return snap.get("max_c") if kind == "high" else snap.get("min_c")


def bucket_dead(ext, kind, lo, hi):
    """Has the daily extreme already moved PAST this bucket for good?

    The daily max only ever rises and the daily min only ever falls, so once
    the running extreme is outside the bucket on the monotonic side, the
    bucket can never come back. `ext` must already be in the bucket's unit
    (see observed_extreme). Markets settle on whole degrees, so round first.
    """
    if ext is None:
        return False
    e = round(ext)
    if kind == "high":
        return hi is not None and e > hi
    return lo is not None and e < lo


def bucket_favored(ext, kind, lo, hi):
    """Is this bucket currently WINNING — i.e. the running extreme sits inside
    it right now? On a locked bucket (extreme set, peak/trough passed) this is
    what decides which side of the market the data favors: inside => YES,
    outside => NO. `ext` must be in the bucket's unit. None when unknowable.

    Note this is only meaningful together with a lock check: mid-morning a
    high bucket below the eventual peak is "inside" and still doomed.
    """
    if ext is None:
        return None
    e = round(ext)
...
class PolyRewardsExec:
    ...
    def check(self, scan_results, require_confidence=True):
        ...
        for r, station, city, kind, date in matched:
            ...
            # `confidence` grades how accurate the YIELD ESTIMATE is, not how
            # risky the market is. It reads "low" when the book is too thin to
            # measure our share against — which is exactly what a decided
            # bucket's book looks like, and exactly where a small quoter earns
            # the LARGEST share of a pool. Measured 2026-08-04: 297 of 372
            # scanned markets were confidence="ok", but they were the
            # CONTESTED ones; every market that passed this lock check was
            # "low", so requiring "ok" here left the tradeable set empty (9
            # cycles, 0 entries).
            #
            # Callers whose safety does not rest on the estimate — the two-leg
            # executor, whose protection is lock + market consensus + wide
            # two-sided quotes + auto-complete + a capital cap — pass
            # require_confidence=False. The single-leg path, which DOES size
            # and rank off the estimate, keeps the strict default.
            if require_confidence and r.get("confidence") != "ok":
            ...
            rec = {
                ...
                # carried so downstream ledgers/reports can segment results by
                # how trustworthy the yield estimate was at entry
                "confidence": r.get("confidence"),
                "no_competition_detected": r.get("no_competition_detected"),
            }
            self._persist(rec)
    ...
    def check_fills(self, client):
        ...
        for order_id, o in tracked.items():
            ...
            # Read the extreme in the BUCKET'S unit — this was hardcoded to
            # Celsius and compared against °F bounds on every US market, so the
            # stop-loss below could not fire correctly there (fixed 2026-08-04).
            ext = observed_extreme(st, o["kind"], bucket.get("unit"))
            if ext is None:
                continue
            ext_s = round(ext)
            dead = bucket_dead(ext, o["kind"], lo, hi)
            trend_dead = False
