...
class PolyClient:
    ...
    def place_sell_fok_at(self, token_id: str, limit_c: float, size: float, fee_bps: int,
                          neg_risk: Optional[bool] = None) -> float:
        """
        True FOK sell: fills the FULL `size` at `limit_c` or better, or fills
        NOTHING. Unlike place_sell_fok() above (FAK at a 1c giveaway limit,
        built for "salvage anything before a dead bucket's bids vanish"),
        this either exits near the intended price or leaves the position
        completely untouched — no silent sweep down the book into a much
        worse average fill on a partial match.

        For take-profit exits only: a kill here just means "the full size
        wasn't there at this price" — the position stays open and gets
        re-evaluated (and can retry this sell) on the next refresh cycle.
        Do NOT use this for an emergency/naked-leg unwind; that needs FAK.

        Returns filled share count — 0.0 on a kill. Sets
        self._last_sell_proceeds_usd / self._last_fill_price_cents from the
        REAL fill on success, same convention as place_sell_fok().
        """
        self._last_sell_proceeds_usd = None
        self._last_fill_price_cents = None
        want = int(size)
        if want <= 0:
            return 0.0
        if DRY_RUN:
            log.info("[DRY RUN] Poly FOK sell token=...%s x%s @ %.1fc", token_id[-6:], want, limit_c)
            self._last_sell_proceeds_usd = want * limit_c / 100.0
            self._last_fill_price_cents = limit_c
            return float(want)
        if self._clob is None:
            log.error("Poly v2 client unavailable — cannot sell")
            return 0.0
        try:
            from py_clob_client_v2.clob_types import (OrderArgs, OrderType,
                                                      PartialCreateOrderOptions)
            ...
            log.error("py-clob-client-v2 not installed")
            return 0.0
        price = max(0.01, round(limit_c / 100.0, 2))
        args = OrderArgs(price=price, size=want, side=SELL, token_id=token_id)
        opts = (PartialCreateOrderOptions(neg_risk=neg_risk)
                if neg_risk is not None else None)
        signed = self._clob.create_order(args, opts)
        resp = self._clob.post_order(signed, OrderType.FOK)
        ...
        # (reverse of a buy) — see place_sell_fok()'s _submit_sell for why.
        try:
            making = float(resp.get("makingAmount", "") or "")
            taking = float(resp.get("takingAmount", "") or "")
            if making > 0:
                self._last_fill_price_cents = (taking / making) * 100.0
                self._last_sell_proceeds_usd = taking
        except (TypeError, ValueError):
            pass
        return filled
    ...
    def _connect_ws(self):
        ...
        # Protocol-level ping/pong keeps the connection alive — an app-level
        # "PING" text frame was tried here previously but the server now kills
        # the socket immediately on receiving it (verified: idle connections
        # survive 100s+ on protocol ping alone; sending "PING" drops in <1s).
