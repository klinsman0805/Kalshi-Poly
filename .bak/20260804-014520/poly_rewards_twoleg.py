...
from modules.poly_rewards_live import place_limit
...
class PolyTwoLeg:
    ...
    def _enter(self, plan):
        ...
        if POLY_TWOLEG_LIVE and not (yes_oid and no_oid):
            # A half-placed pair is the exact naked-leg state this design
            # exists to avoid — never leave one side resting alone.
            self.on_log("!", "[twoleg] one leg failed to place — cancelling the other, "
                             "refusing to rest a single naked leg")
            return
    ...
    def _close(self, reason):
        self._persist({"type": "twoleg_closed", "id": self.pos["id"],
