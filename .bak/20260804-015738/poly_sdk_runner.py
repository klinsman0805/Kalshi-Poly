...
async def _handle(cmd):
    ...
    if action == "cancel":
        oid = cmd["order_id"]
        res = await client.cancel_order(order_id=oid)
        canceled = list(getattr(res, "canceled", None) or [])
        not_canceled = _jsonable(getattr(res, "not_canceled", None))
        # cancel_limit's purpose is "ensure nothing is left resting on the
        # book". So "ok" is true when the order is confirmed cancelled, when
        # nothing was reported un-cancelled, OR when the only reason it wasn't
        # cancelled is that it is already gone (filled/cancelled/not found) —
        # in every one of those cases there is nothing left to leak. A genuine
        # failure (auth, network, rate limit) leaves a real reason and ok=False.
        gone_markers = ("can't be found", "cannot be found", "not found",
                        "already", "matched", "cancel")
        ...
        ok = (oid in canceled) or (not not_canceled) or all_gone
