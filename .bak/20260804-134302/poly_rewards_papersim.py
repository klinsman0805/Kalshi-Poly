...
class PolyRewardsPaperSim:
    ...
    def _check_filled(self):
        ...
        if not st:
            return
        ext = st.get("max_c") if self.pos["kind"] == "high" else st.get("min_c")
        if ext is None:
            return
        ext_s = round(ext)
        lo, hi = bucket.get("lo"), bucket.get("hi")
        dead = ((self.pos["kind"] == "high" and hi is not None and ext_s > hi) or
                (self.pos["kind"] == "low" and lo is not None and ext_s < lo))
        trend_dead = False
        if not dead:
